...
public class TreeEnsembleClassificationLearnerNodeFactory2 extends
    ...
    private static final String FULL_DESCRIPTION = """
            <p> Learns an ensemble of decision trees (such as random forest* variants). Typically, each tree is
                ...
                (fingerprint), byte, or double vector. The output model describes an ensemble of decision tree models
                and is applied in the corresponding predictor node using the selected aggregation mode to aggregate the
                votes of the individual decision trees. </p> <p> The following configuration settings learn a model that
                is similar to the <a href="http://en.wikipedia.org/wiki/Random_forest">random forest</a>™ classifier
                described by Leo Breiman and Adele Cutler: <ul> <li>Tree Options - Split Criterion: Gini Index</li>
                <li>Tree Options - Limit number of levels (tree depth): unlimited</li> <li>Tree Options - Minimum node
                size: unlimited</li> <li>Ensemble Configuration - Number of models: Arbitrary (arguably, random forest
                does not overfit)</li> <li>Ensemble Configuration - Data Sampling: Use all rows (fraction = 1) but
                choose sampling with replacement (bootstrapping) </li> <li>Ensemble Configuration - Attribute Sampling:
                Sample using a different set of attributes for each tree node split; usually square root of number of
                attributes - but this can vary </li> </ul> Experiments have shown that results on different datasets are
                very similar to the <a href="http://cran.r-project.org/web/packages/randomForest/">random forest
                implementation available in R</a>. </p> <p> Decision tree construction takes place in main memory (all
                data and all models are kept in memory). </p> <p> The missing value handling corresponds to the method
                described <a href="https://github.com/dmlc/xgboost/issues/21">here</a>. The basic idea is that for each
                split to try and send the missing values in every possible direction and the one yielding the best
                ...
                during testing. </p> <p> The tree ensemble nodes now also support binary splits for nominal columns.
                Depending on the kind of problem (two- or multi-class), different algorithms are implemented to enable
                the efficient calculation of splits. <ul> <li>For two-class classification problems the method described
                in section 9.4 of "Classification and Regression Trees" by Breiman et al. (1984) is used.</li> <li>For
